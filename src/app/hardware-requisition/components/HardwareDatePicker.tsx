...
interface HardwareDatePickerProps {
  value: string;
...
export default function HardwareDatePicker({
  value,
  onChange,
  isNonWorkingDay,
  placeholder = 'তারিখ নির্বাচন করুন',
  disabled = false
}: HardwareDatePickerProps) {
  ...
  useEffect(() => {
    const handleOutsideClick = (event: MouseEvent) => {
      ...
      if (!target.closest('.hardware-calendar-picker-container')) {
        setIsOpen(false);
      }
    };
    if (isOpen) {
  ...
  return (
    <div className="relative hardware-calendar-picker-container font-sans w-full">
      <button
        type="button"
        disabled={disabled}
