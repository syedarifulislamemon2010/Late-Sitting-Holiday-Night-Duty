...
export default function EmployeesPage() {
  ...
  const filteredEmployees = employees.filter(emp => {
    const matchesSearch = emp.name.toLowerCase().includes(searchQuery.toLowerCase()) || 
                          ...
                          (emp.bankId && emp.bankId.toLowerCase().includes(searchQuery.toLowerCase())) ||
                          (emp.fileNo && emp.fileNo.toLowerCase().includes(searchQuery.toLowerCase()));
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 glass-card p-4 rounded-2xl">
            <div className="flex flex-1 flex-col sm:flex-row gap-3">
              ...
              <div className="relative flex-1">
                ...
                <input
                  type="text"
                  placeholder="কর্মকর্তার নাম, পদবী, ব্যাংক আইডি বা ফাইল নম্বর দিয়ে খুঁজুন..."
                  value={searchQuery}
                    ...
                    <div className="space-y-1.5 text-xs text-slate-500 dark:text-slate-400 pt-1 border-t border-slate-100 dark:border-slate-800/80">
                      {emp.bankId && (
                        <p className="flex items-center gap-2">
                          ...
                          <span className="font-semibold text-slate-400">ব্যাংক আইডি:</span> {emp.bankId}
                        </p>
                      )}
                      {emp.fileNo && (
                        <p className="flex items-center gap-2">
                          <Hash size={12} className="text-slate-400" />
                          <span className="font-semibold text-slate-400">ফাইল নম্বর:</span> {emp.fileNo}
                        </p>
                      )}
                    </div>
                  </div>
              ...
              <div key={cell.id} className="glass-card p-6 rounded-2xl flex flex-col justify-between hover:border-slate-300 dark:hover:border-slate-700 transition-all">
                <div className="space-y-3">
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                    {cell.description || 'এই সেলের কোনো অতিরিক্ত বিবরণী লিপিবদ্ধ করা নেই।'}
                  </p>
                </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-lg overflow-hidden border border-slate-200 dark:border-slate-800 shadow-xl">
            ...
            <form onSubmit={handleEmpSubmit} className="p-6 space-y-4">
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">ফাইল নম্বর</label>
                <input
                  type="text"
                  placeholder="যেমন: F-9034"
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-md overflow-hidden border border-slate-200 dark:border-slate-800 shadow-xl">
            ...
            <form onSubmit={handleCellSubmit} className="p-6 space-y-4">
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">সেলের বিবরণ</label>
                <textarea
                  rows={3}
                  placeholder="যেমন: আর৯ সেল প্রশাসন বা টেকনিক্যাল কাজের দায়িত্বপ্রাপ্ত।"
