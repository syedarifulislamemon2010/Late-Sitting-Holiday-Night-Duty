...
export class OfficeOrderRepository {
  ...
  static async linkDutiesToOrderRef(dutyIds: number[], orderRef: string | null, tx?: any) {
    const client = tx || db;
  ...
  static async clearDutiesOrderRef(orderRef: string, tx?: any) {
    const client = tx || db;
    return client.update(dutiesTable)
      .set({ orderRef: null })
