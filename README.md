...
### 13.2 Multi-Factor Authentication (MFA) Design (Planned Roadmap)
To comply with enterprise banking security guidelines, the following **MFA (TOTP-based)** specifications are planned for future implementation:
...
### 13.3 Security & Account Lockout Policies (Planned Roadmap)
* **Account Lockout (Roadmap):** To mitigate automated credential brute-forcing:
  * An account will be automatically locked after **5 consecutive failed login attempts**.
  * The lockout duration is set to **30 minutes**.
  * Each lockout event will register a high-priority security event log in the Audit database.
* **Session Security Controls (Roadmap):**
  * **Idle Session Timeout:** Sessions will automatically invalidate after **15 minutes** of operator inactivity.
  * **Absolute Session Timeout:** Active session tokens will possess an absolute validity window of **8 hours** to prevent persistent session hijack vectors.
...
* **Data Encryption Standards:**
  ...
  * **At-Rest (Roadmap):** Database storage and backup exports (`postgres_dump.json`) will be encrypted using AES-256-CBC.
...
* **API Contract Tests:** Validates backend REST endpoint responses, headers, RBAC scope enforcement, and JSON payload structures:
  ...
  ```
* **E2E Playwright Tests (Planned Roadmap):** Automates real-world browser paths (operator logins, manual roster scheduling, form overrides, and silent PDF printing) inside headless browsers:
  ```bash
  # npx playwright test
  ```
...
To guarantee runtime visibility, the application plans integration with an enterprise-grade monitoring stack (Planned Roadmap):
...
│   │   │   ├── auth/
│   │   │   │   └── [...nextauth]/
│   │   │   │       └── route.ts           # NextAuth.js configurations
│   │   │   ├── cells/
│   │   │   │   ├── route.ts               # Lists cells or handles bulk cell CSV imports
│   │   │   │   └── [id]/
│   │   │   │       └── route.ts           # Handles single-cell updates
│   │   │   ├── documents/
...
│   │   │   │       └── route.ts           # Compiles printable duty roster orders
│   │   │   ├── duties/
...
│   │   │   │   └── parse/
│   │   │   │       └── route.ts           # OCR script for importing holidays
│   │   │   ├── leaves/
...
│   │   │   ├── profile/
│   │   │   │   └── route.ts               # Fetches session profile data once
│   │   │   ├── trash/
│   │   │   │   └── route.ts               # Restores soft-deleted items
│   │   │   ├── upload/
│   │   │   │   └── route.ts               # Handles file uploads
│   │   │   └── users/
│   │   │       ├── route.ts               # Lists and registers bank cell operators
│   │   │       └── [id]/
│   │   │           └── route.ts           # Alters operator permissions, cellDuties configurations
│   │   ├── audit/
...
│   │   └── users/
│   │       └── page.tsx                   # Admin manager for mapping users to cell assignments
│   ├── components/
...
│   │   └── ProfileContext.tsx             # Fetches session profile once
│   ├── db/
...
│   │       ├── 0000_tired_menace.sql      # Initial tables
│   │       ├── 0001_huge_tusk.sql         # Leave configurations
│   │       ├── 0002_stale_bucky.sql       # Lunch bill mappings
│   │       ├── 0003_nice_vance_astro.sql  # Manual documents
│   │       ├── 0004_tan_purple_man.sql    # Audit log tables
│   │       ├── 0005_amazing_black_bolt.sql # cellDuties column user mapping
│   │       ├── 0006_clever_jasper_sitwell.sql # employee userId link column
│   │       └── meta/                      # Kit journal schema snapshots
│   ├── hooks/
│   │   └── usePageData.ts                 # Page loading utility hook
│   ├── lib/
│   │   ├── audit.ts                       # Appends details to audit.log file
│   │   ├── auth-wrapper.ts                # Resolves user sessions
│   │   ├── bengali-converter.ts           # Bidirectional font mapper engine
│   │   ├── db.ts                          # PostgreSQL client connector
│   │   ├── errors.ts                      # Global error codes catalog
...
│   ├── permissions/
│   │   └── rbac.ts                        # Configures matrix for ADMIN, USER, EMPLOYEE
│   ├── repositories/
│   │   ├── duty.repository.ts             # Drizzle duty mutations
│   │   ├── employee.repository.ts         # Drizzle employee mutations
│   │   ├── holiday.repository.ts          # Drizzle holiday database queries
│   │   ├── leave.repository.ts            # Drizzle leave database queries
│   │   ├── officeOrder.repository.ts      # Drizzle office order database queries
│   │   └── user.repository.ts             # Drizzle user database queries
│   └── services/
│       ├── __tests__/                     # Automated test suites
│       │   ├── duty.service.test.ts
│       │   ├── leave.service.test.ts
│       │   └── officeOrder.service.test.ts
│       ├── duty.service.ts                # Allowance calculations
│       ├── employee.service.ts            # Employee updates
...
* **Analytics Dashboard & Employee Self-Service Portal**: Introduced a modern, comprehensive data analytics panel under `/analytics` and `/api/analytics` equipped with dynamic `recharts` visualizers.
  * **Strict Financial Privacy Rules**: Enforces bank-grade confidentiality on individual allowance earnings. Employees can view only their personal monthly allowance trend ("আমার প্রাপ্ত ভাতার ধারা") and personal leaves. Admins/Users can see bank-wide monthly totals and cell-wise budget breakdowns, but individual allowances are hidden bank-wide (the Top Performers chart displays duty count, never financial amounts).
  * **KPI Summary Cards**: Renders interactive, styled metric cards showing overall completed duties, overall released bills, and personal bill counts and earnings for any user mapped to an employee record (e.g. Bahar seeing his personal totals).
  * **Bill Release Trends & Counts**: Queries bill-related `OfficeOrder` records (`category LIKE 'BILL_%'`) to display daily release statistics (e.g., "June 10: 9 bills") and a leaderboard of released bills per employee (e.g., "Bahar: 4 bills"), filterable by month or year and visible to everyone.
  * **Duty Type Dropdown & Combined View**: Upgraded the filter bar (renamed "ডিউটি ও বিল ফিল্টারিং") to include a "ডিউটি ধরন" dropdown (Late Sitting, Holiday, Night Shift) defaulting to "সিলেক্ট করুন", which programmatically displays combined top performers across all duty types if no specific type is selected.
  * **RBAC Route Permissions**: Expanded `canAccessRoute` in `rbac.ts` and `AuthGuard.tsx` client-side guards to grant the `EMPLOYEE` role access to `/analytics`, `/analytics/*`, and `/api/analytics/*` paths, adding the link (with the `TrendingUp` icon) to their Sidebar.
